...
quarter_texts = [
    anim.create_sans_bold_text("1/4", square.points[0], size / 4).set_opacity(0.).center_on(square)
    for square, size in zip(central_tower_squares, square_sizes)
]

exponent_texts = [
    anim.create_sans_bold_text(str(exponent+1), quarter.exponent_pos(), quarter.get_font_size() / 2).set_opacity(0.)

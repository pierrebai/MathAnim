...
class animation(anim.animation):
    ...
    def _gen_star(self, scene: anim.scene):
        ...
        """
        dot = self.inner_dots_pos[0][0]
        center = self.inner_centers[0]
        star_segments = 240 // max(self.skip, 1)
        pts = []
        # for i in range(star_segments * self.skip + 1):
        #     outer_angle = 360. * self.sides * float(i) / float(star_segments)
        #     inner_angle = outer_angle / self.inner_circle_ratio
        #     new_pos = anim.point(anim.trf.rotate_around(dot, center, inner_angle))
        #     new_pos = anim.point(anim.trf.rotate_around(new_pos, anim.point(0., 0.), outer_angle))
        #     pts.append(new_pos)
        ...
        def prep_anim(scene: anim.scene, animator: anim.animator):
            ...
            for which_inner in range(self.inner_count):
                ...
                for which_dot in range(self.dots_count):
                    ...
                    animator.animate_value(inner_angle, 0., 2. * self.animation_speedup, rot_dot)
